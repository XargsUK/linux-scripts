...
if(defined($opts{'noDeferralDetail'})) {
    ...
    warn "$progName: \"no_deferral_detail\" is depreciated, use \"deferral_detai                                   l=0\" instead\n"
}
...
if(defined($opts{'noSMTPDWarnings'})) {
    ...
    warn "$progName: \"no_smtpd_warnings\" is depreciated, use \"smtpd_warning_d                                   etail=0\" instead\n"
}

# If --detail was specified, set anything that's not enumerated to it
if(defined($opts{'detail'})) {
    foreach my $optName (qw (h u bounceDetail deferralDetail smtpDetail smtpdWar                                   nDetail rejectDetail)) {
...
while(<>) {
        ...
        /^(...) {1,2}(\d{1,2}) (\d{2}):(\d{2}):(\d{2}) \S+ (.+)$/o) == 6)
    {
        ...
        next unless((($msgYr, $msgMon, $msgDay, $msgHr, $msgMin, $msgSec, $logRm                                   dr) =
        ...
        /\/cleanup\[\d+\]: .*?\b(reject|warning|hold|discard): (header|body) (.*                                   )$/o) == 3)
    {
        ...
        if($logRmdr =~ /\[\d+\]: \w+: client=(.+?)(,|$)/o) {
            ...
            if($logRmdr =~ /: connect from /o) {
                ...
                my ($pid, $hostID) = $logRmdr =~ /\/smtpd\[(\d+)\]: disconnect f                                   rom (.+)$/o;
                if(exists($connTime{$pid})) {
                    $hostID = gimme_domain($hostID);
                    my($d, $h, $m, $s) = Delta_DHMS(@{$connTime{$pid}},
                        $msgYr, $msgMon + 1, $msgDay, $msgHr, $msgMin, $msgSec);
                    delete($connTime{$pid});    # dispose of no-longer-needed it                                   em
                    my $tSecs = (86400 * $d) + (3600 * $h) + (60 * $m) + $s;

                    ++$smtpdPerHr[$msgHr][0];
                    $smtpdPerHr[$msgHr][1] += $tSecs;
                    $smtpdPerHr[$msgHr][2] = $tSecs if($tSecs > $smtpdPerHr[$msg                                   Hr][2]);
...
sub print_problems_reports {
    unless($opts{'deferralDetail'} == 0) {
        print_nested_hash(\%deferred, "message deferral detail", $opts{'deferral                                   Detail'}, $opts{'q'});
    }
    unless($opts{'bounceDetail'} == 0) {
        print_nested_hash(\%bounced, "message bounce detail (by relay)", $opts{'                                   bounceDetail'}, $opts{'q'});
    }
    unless($opts{'rejectDetail'} == 0) {
        print_nested_hash(\%rejects, "message reject detail", $opts{'rejectDetai                                   l'}, $opts{'q'});
        print_nested_hash(\%warns, "message reject warning detail", $opts{'rejec                                   tDetail'}, $opts{'q'});
        print_nested_hash(\%holds, "message hold detail", $opts{'rejectDetail'},                                    $opts{'q'});
        print_nested_hash(\%discards, "message discard detail", $opts{'rejectDet                                   ail'}, $opts{'q'});
    }
    unless($opts{'smtpDetail'} == 0) {
        print_nested_hash(\%smtpMsgs, "smtp delivery failures", $opts{'smtpDetai                                   l'}, $opts{'q'});
    }
    unless($opts{'smtpdWarnDetail'} == 0) {
        print_nested_hash(\%warnings, "Warnings", $opts{'smtpdWarnDetail'}, $opt                                   s{'q'});
    }
